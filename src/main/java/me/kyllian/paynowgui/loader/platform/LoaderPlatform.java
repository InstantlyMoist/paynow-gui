...
public class LoaderPlatform implements PayNowPlatform {

    private final PayNowLoaderMod mod;
    ...
    public LoaderPlatform(PayNowLoaderMod mod) {
        this.mod = mod;
        this.scheduler = new LoaderScheduler(mod);
        //? if fabric {
        this.dataFolder = FabricLoader.getInstance().getConfigDir().resolve("paynow-gui").toFile();
        //?} else {
        /*this.dataFolder = FMLPaths.CONFIGDIR.get().resolve("paynow-gui").toFile();
        *///?}
